...
class Environment(object):
	...
	#
	identifier = ""
	initial_assets = 0.0 # the initial assets are needed to determine the fire-sale price in bank.liquidate_assets
	...
	def get_state(self,  time):
		...
		for parameter in self.parameters.parameters:
			...
			if (int(time) >= int(validFrom)) and (int(time) <= int(validTo)): # we have a valid parameterset
				...
				if parameter['type']=='pBank':
					self.state.pBank = float(parameter['value'])
